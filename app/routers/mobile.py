...
def get_batch_full_details(batch_id: str, db: RealDictCursor = Depends(get_db)):
    ...
    # 3. Calculate Status & Integrity
    from app.services.verification_service import VerificationService
    
    # Check Integrity (Blockchain vs DB)
    integrity = VerificationService.verify_batch_integrity(batch_id, db)
    
    # Determine Logic Status
    final_status = "AUTHENTIC"
    status_message = "Verified on Blockchain."
    
    if integrity['status'] == 'TAMPERED':
        final_status = "TAMPERED"
        status_message = integrity['message']
    elif batch['current_status'] == 'UNDER_INVESTIGATION':
        final_status = "UNDER_INVESTIGATION"
        status_message = "Batch is flagged for surveillance."
    elif batch['current_status'] == 'QUARANTINE':
        final_status = "QUARANTINE"
        status_message = "Batch pending FDA Approval."
    ...
    return {
        ...
        "integrity_proof": {
            "blockchain_status": integrity['status'],
            "last_tx": events[0]["blockchain_tx_id"] if events else None,
            "tamper_details": integrity.get("details")
        }
    }
...
def download_certificate(batch_id: str, db: RealDictCursor = Depends(get_db)):
    """
    Returns downloadable certificate data. Preferentially uses REAL COA from DB if available.
    """
    ...
    # 3. Check for Real COA
    from app.services.coa_service import COAService
    real_coa = COAService.get_coa(batch_id, db)
    ...
    if real_coa:
        # Use Real Data
        qc_data = real_coa['analysis_results']
        qc_data['status'] = "PASS" if real_coa['conclusion'] == 'Complies with specifications' else "REVIEW"
        qc_data['conclusion'] = real_coa['conclusion']
        qc_data['signature_path'] = real_coa['signature_path']
        qc_data['coa_id'] = f"COA-{real_coa['coa_id']}"
    else:
        # Fallback / Simulation
        qc_data = {
            "assay": "Pending Analysis", 
            "status": "PENDING_QC",
            "note": "Official COA not yet generated by Quality Control."
        }
        # Try to find real QC metrics in event metadata as a backup
        for e in events:
    ...
    report = {
        "report_header": {
            ...
            "certificate_id": qc_data.get('coa_id', f"CERT-{batch['batch_id']}"),
        ...
        "quality_assurance": {
            ...
            "regulatory_status": "FDA Compliant" if batch.get('fda_approval_status') == 'APPROVED' else "Pending Approval"
        },
        "blockchain_verification": {
            "ledger_type": "ETHEREUM (GANACHE)",
            "contract_address": "0xB8D3153439f7E9cb9eBa9360faB117C6137584B6", 
